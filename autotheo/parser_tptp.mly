%{ open Parser_types
 ...
 let disquote_string  str = 
   if String.length str >= 2 && str.[0] = '\'' && str.[String.length str -1] = '\''
   then
...
/* key words*/
%token <string> CNF FOF Include Axiom Hypothesis Definition Assumption 
  Conjecture Negated_conjecture Lemma Theorem Plain Unknown Derived
...
user_type :
   ...
   |Definition {Axiom} |Assumption {Hypothesis} 
   |Conjecture {Globals.seen_conjecture := true; Conjecture}
   |Negated_conjecture {Globals.seen_conjecture := true; Negated_conjecture} 
