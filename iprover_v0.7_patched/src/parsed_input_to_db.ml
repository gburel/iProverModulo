...
let cnf_formula_to_db ?(oneway=false) parsed_formula =  
  let clause_tmp = 
    list_of_literals_to_clauses (get_list_of_literals parsed_formula) in 
  let new_clause = (Clause.normalise ~oneway term_db_ref clause_tmp) in
...
(* to have correct has_conj_symb *)
  let rec f list = 
    ...
    |Formula(CNF,name,formula_type,parsed_formula,
	    ...
	    |UserType(Axiom) -> 
               axiom_list:= parsed_formula::!axiom_list;
                f tl   
...
(*  let non_conj_cnf = *)
      List.iter (fun p -> 
		   let _ = cnf_formula_to_db 
		     ~oneway:(!Options.current_options.Options.modulo) p in ()) 
	(!axiom_list);
