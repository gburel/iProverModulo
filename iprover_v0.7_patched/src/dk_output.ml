...
let dk_rules = ref []
let definable_symbols = ref []

let uprop = DVar (Qid ("FOL","o"))
let eprop = DVar (Qid ("FOL","eps"))
let bot_var = Qid ("FOL","bot")
let iota = DVar (Qid ("FOL","i"))
let dummy = DVar (Qid ("FOL","const"))
let ordk = DVar (Qid ("FOL","or"))
let fa_i = DVar (Qid ("FOL","fa_i"))
let notdk = DVar (Qid ("FOL","not"))
...
let rec term_to_dk ?(sig_pref=true) term = match term with
  ...
  | Fun (f, ts, _) ->
     if Symbol.equal f Symbol.symb_neg then
       term_list_to_dk sig_pref notdk ts
     else term_list_to_dk sig_pref (DVar(if sig_pref then sig_id (symbol_name f) else Id(symbol_name f))) ts
...
let literal_to_dktype ?(sig_pref=true) p =
  if Term.is_neg_lit p then
    DPi(Id "_", DPi(Id "_", DApp(eprop, term_to_dk ~sig_pref (get_atom p)), DApp(eprop, DVar bot_var)), DApp(eprop, DVar bot_var))
  else
    DPi(Id "_", DApp(eprop, term_to_dk ~sig_pref p), DApp(eprop, DVar(bot_var)))
...
let ground_literal_to_dk ?(renaming_list=[]) free_vars p =
  if Term.is_neg_lit p then
     DPi(Id "_", DApp(eprop, ground_term_to_dk ~renaming_list free_vars (get_atom p)), DApp(eprop, DVar bot_var))
  else
    DApp(eprop, ground_term_to_dk ~renaming_list free_vars p)
...
let rec fa_to_dk vars t =
  match vars with
    [] -> t
  | (v,_) :: q ->
     fa_to_dk q (DApp(fa_i, DFun (Id (Var.to_string v), iota, t)))

let rec or_to_dk lits = match lits with
    [] -> DVar bot_var
  | [l] -> term_to_dk l
  | l::q -> DApp(DApp(ordk, term_to_dk l), or_to_dk q)

let clausalform_to_dk c =
  let lits = Clause.get_literals c in
  DApp(eprop, fa_to_dk (variables_in_clause c) (or_to_dk lits))
...
let symbol_to_dk s l =
  if Symbol.is_input s || Lib.(match Symbol.get_property s with
    ...
    match s.stype with
    | Fun -> l @ [
      let n, t = Id (symbol_name s), arity iota (Symbol.get_arity s) in
      if List.mem s !definable_symbols then
	Defdecl(n,t) else
	Declaration(n, t) ]
    | Pred -> l @ [
      let n, t = Id (symbol_name s), arity uprop (Symbol.get_arity s) in
      if List.mem s !definable_symbols then
	Defdecl(n,t) else
	Declaration(n, t)]
    | _ -> l
...
let literal_to_proof l =
  DUFun(Id "clcl", DApp(DApp(literal_to_dkvar l, DVar (Id "clcl")), DVar (Id "clcz")))

let rec clc_rec lits =
  match lits with
    [] -> failwith "Empty clause as input ??"
  | [l] -> literal_to_proof l
  | l :: q ->
     DUFun(Id "clcsb", DApp(DApp(DApp(DVar (Id "clcsb"), DVar (Id "clcz")),
				 literal_to_proof l),
			    clc_rec q))

let clc_core core lits =
  match lits with
    [] -> failwith "Empty clause as input ??"
  | [l] -> DApp(DApp(literal_to_dkvar l, core), DVar (Id "clcz"))
  | l :: q -> DApp(DApp(DApp(core, DVar (Id "clcz")), literal_to_proof l),
			    clc_rec q)


let clausalform_to_clause core c =
  let lits = Clause.get_literals c in
  let vars = variables_in_clause c in
  abstract_variables (
    abstract_literals (
      DFun(Id "clcz", uprop,
	    clc_core (apply_variables core vars vars) lits
      ))
            lits
  ) vars
...
and history_to_dk_aux clause decls =
  ...
  with Not_found ->
    ...
    let res =      name,
      ...
      | Input ->
	 let form = Id("input"^ string_of_int !clause_counter) in
	 push
	   (push decls (Declaration(form, clausalform_to_dk clause)))
	   (Definition(name, clause_to_dk clause, clausalform_to_clause
	     (DVar(form)) clause))
	...
	let res = DApp(k, DFun(Id "h", DApp(eprop, term_to_dk lit),
...
let add_split_to_dk lit lits =
  ...
  dk_rules :=
    Rules([[], DApp(eprop, term_to_dk ~sig_pref:false lit), rhs])
  ::!dk_rules
