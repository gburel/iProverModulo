...
let dedukti_proof_fun clause =
  ...
  in
  Dkterm.pp_external ();
  let generated_string suffix =
    let open Format in
    fprintf str_formatter "@[(; @[<v>@[Module %s_%s@]@,@[Dedukti file generated by iProverModulo@]@,@[Git commit id: %s @]@];)@]@."
    prefix suffix Git_version.commit_id
    ;
    flush_str_formatter ()
  in
  output_string outch_sig (generated_string "sig");
  ...
  output_string outch_prf (generated_string "prf");
