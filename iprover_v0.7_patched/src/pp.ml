...
let strbrk s =
  let rec aux p n =
    if n < String.length s then 
      if s.[n] = ' ' then
	if p=n then (Stream.iapp (spc ()) (aux (n+1) (n+1)))
	else (Stream.iapp (str (String.sub s p (n-p))) (Stream.iapp (spc ()) (aux (n+1) (n+1))))
      else aux p (n+1)
    else if p=n then (Stream.of_list [ ]) else  str (String.sub s p (n-p)) 
  in aux 0 0
...
let warnbody strm =
  (Stream.iapp ( warnstart()) (Stream.iapp  (hov 0 (str "Warning: " ++ strm))  (warnend())))

(* pretty printing functions WITHOUT FLUSH *)
let pp_with ft strm =
  pp_dirs ft (Stream.of_list [ Ppdir_ppcmds strm ])

let ppnl_with ft strm =
  pp_dirs ft (Stream.of_list [ Ppdir_ppcmds (Stream.iapp strm (Stream.of_list [  Ppcmd_force_newline ]))])
